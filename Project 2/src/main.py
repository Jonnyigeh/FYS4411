...
if __name__=="__main__":
    ...
    state = RBM(n_visible=config.n_visible_nodes,
                       n_hidden=config.n_hidden_nodes,
                       temperature=config.temperature)
    
    # Set the hamiltonian for our system
    hamiltonian = onedim_ising_model(nparticles=config.n_visible_nodes,
                                     J=config.J,
                                     mu=config.mu)
    # breakpoint()

    # Choose, and initialize sampler
    sampler = Metropolis(n_procs=config.n_procs,
                         hamiltonian=hamiltonian)
    
    # Initialize the optimizer
    sgd_optimizer = sgd(state=state,
    ...
    for nv in [3.0, 5.0, 10, 12]:
        config.nv = nv
        state.reset_params()
        le["n_v = " + str(nv)], var["T= " + str(nv)], stde["T= " + str(nv)], res["T= " + str(nv)], *_ = sgd_optimizer.run_optimizer()

...
class onedim_ising_model(hamiltonian):
    ...
    def local_energy_long_int(self, spin_configuration, n_v=5):
        """
        ...
        for i in range(len(spin_configuration)):
            ...
            for j in range(i + 1, len(spin_configuration)):
                energy += (
                    -Jij(i, j, n_v=n_v) * spin_configuration[i] * spin_configuration[j]
                )

        return energy

    def local_energy(self, spin_configuration):
        """
        Evaluate the Hamiltonian, and find local energy of system
        ...
        for i in range(len(spin_configuration) - 1):
            energy += (
                -self.J * spin_configuration[i] * spin_configuration[i + 1]
                - self.mu * spin_configuration[i]
            )
